...
//    #define wglSwapIntervalEXT glad_wglSwapIntervalEXT
    ...
    int GL_Init(LE_Display *display)
    {
        ...
        hDC = GetDC((HWND)display->platformHandle);
    ...
    void GL_Update(LE_Display *display)
    {
        SwapBuffers(hDC);
    }
    ...
    void GL_Cleanup(LE_Display *display)
    {
        wglMakeCurrent(NULL, NULL);
        wglDeleteContext(hGLRC);
        ReleaseDC((HWND)display->platformHandle, hDC);
    }
    ...
    void GL_Init(LE_Display *display)
    {
        ...
        visualInfo = glXChooseVisual((Display *)display->platformConnection, 0, attributes);
        if (visualInfo == NULL) 
        {
            printf("No appropriate visual found\n");
        }
        glContext = glXCreateContext((Display *)display->platformConnection, visualInfo, NULL, GL_TRUE);
        glXMakeCurrent((Display *)display->platformConnection, (Window)display->platformHandle, glContext);
    ...
    void GL_Update(LE_Display *display)
    {
	   glXSwapBuffers((Display *)display->platformConnection, (Window)display->platformHandle);
    }

    void GL_Cleanup(LE_Display *display)
    {
	   glXDestroyContext((Display *)display->platformConnection, glContext);
    }
#endif /* __linux__ */
