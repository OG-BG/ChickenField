...
/*
  ...
  modified by phikill - port support ANSI C 89
...
TPE_Vec3 TPE_vec3KeepWithinBox(TPE_Vec3 point, 
                               TPE_Vec3 boxCenter,
                               TPE_Vec3 boxMaxVect);

TPE_Vec3 TPE_vec3KeepWithinDistanceBand(TPE_Vec3 point, 
                                        TPE_Vec3 center,
                                        TPE_Unit minDistance, 
                                        TPE_Unit maxDistance);

TPE_Vec3 TPE_vec3(TPE_Unit x, 
                  TPE_Unit y, 
                  TPE_Unit z);

TPE_Vec3 TPE_vec3Minus(TPE_Vec3 v1, 
                       TPE_Vec3 v2);

TPE_Vec3 TPE_vec3Plus(TPE_Vec3 v1, 
                      TPE_Vec3 v2);

TPE_Vec3 TPE_vec3Cross(TPE_Vec3 v1, 
                       TPE_Vec3 v2);

TPE_Vec3 TPE_vec3Project(TPE_Vec3 v, 
                         TPE_Vec3 base);

TPE_Vec3 TPE_vec3ProjectNormalized(TPE_Vec3 v, 
                                   TPE_Vec3 baseNormalized);

TPE_Vec3 TPE_vec3Times(TPE_Vec3 v, 
                       TPE_Unit units);

TPE_Vec3 TPE_vec3TimesPlain(TPE_Vec3 v, 
                            TPE_Unit q);
...
static CC_INLINE TPE_Unit TPE_abs(TPE_Unit x);
static CC_INLINE TPE_Unit TPE_max(TPE_Unit a, TPE_Unit b);
static CC_INLINE TPE_Unit TPE_min(TPE_Unit a, TPE_Unit b);
static CC_INLINE TPE_Unit TPE_nonZero(TPE_Unit x);
static CC_INLINE TPE_Unit TPE_dist(TPE_Vec3 p1, TPE_Vec3 p2);
static CC_INLINE TPE_Unit TPE_distApprox(TPE_Vec3 p1, TPE_Vec3 p2);
...
typedef uint8_t (*TPE_CollisionCallback)(uint16_t, 
                                         uint16_t, 
                                         uint16_t, 
                                         uint16_t,
                                         TPE_Vec3);
...
/** Rotates a 3D point by given Euler angle rotation (see rotation
  conventions). */
TPE_Vec3 TPE_pointRotate(TPE_Vec3 point, 
                         TPE_Vec3 rotation);
...
static CC_INLINE TPE_Unit TPE_connectionTension(TPE_Unit length,
...
TPE_Vec3 TPE_rotationRotateByAxis(TPE_Vec3 rotation, 
                                  TPE_Vec3 rotationByAxis);

/** Computes the formula of a 1D collision of rigid bodies. */
void TPE_getVelocitiesAfterCollision(TPE_Unit *v1, 
                                     TPE_Unit *v2, 
                                     TPE_Unit m1,
                                     TPE_Unit m2, 
                                     TPE_Unit elasticity);
...
TPE_Vec3 TPE_rotationFromVecs(TPE_Vec3 forward, 
                              TPE_Vec3 right);

TPE_Joint TPE_joint(TPE_Vec3 position, 
                    TPE_Unit size);
...
uint8_t TPE_jointsResolveCollision(TPE_Joint                *j1, 
                                   TPE_Joint                *j2,
                                   TPE_Unit                 mass1, 
                                   TPE_Unit                 mass2, 
                                   TPE_Unit                 elasticity, 
...
void TPE_bodyGetFastBSphere(const TPE_Body  *body, 
                            TPE_Vec3        *center,
                            TPE_Unit        *radius);

/** Computes the minimum bounding sphere of a body (there is another function
  for a faster approximate bounding sphere). */
void TPE_bodyGetBSphere(const TPE_Body  *body, 
                        TPE_Vec3        *center,
                        TPE_Unit        *radius);

uint8_t TPE_checkOverlapAABB(TPE_Vec3 v1Min, 
                             TPE_Vec3 v1Max, 
                             TPE_Vec3 v2Min,
                             TPE_Vec3 v2Max);
...
uint8_t TPE_bodiesResolveCollision(TPE_Body                 *b1, 
                                   TPE_Body                 *b2,
                                   TPE_ClosestPointFunction env);
...
static CC_INLINE uint8_t TPE_bodyIsActive(const TPE_Body *body);
...
void TPE_makeCenterBox(TPE_Joint      joints[9], 
                       TPE_Connection connections[18],
                       TPE_Unit       width, 
                       TPE_Unit       depth, 
                       TPE_Unit       height, 
                       TPE_Unit       jointSize);
...
void TPE_make2Line(TPE_Joint      joints[2], 
...
TPE_Vec3 TPE_envBox(TPE_Vec3 point, 
                    TPE_Vec3 center, 
                    TPE_Vec3 maxCornerVec,
                    TPE_Vec3 rotation);

TPE_Vec3 TPE_envSphere(TPE_Vec3 point, 
                       TPE_Vec3 center, 
                       TPE_Unit radius);

TPE_Vec3 TPE_envSphereInside(TPE_Vec3 point, 
                             TPE_Vec3 center, 
                             TPE_Unit radius);
...
TPE_Vec3 TPE_envGround(TPE_Vec3 point, 
                       TPE_Unit height);
...
TPE_Vec3 TPE_envLineSegment(TPE_Vec3 point, 
                            TPE_Vec3 a, 
                            TPE_Vec3 b);
...
TPE_Vec3 TPE_envHeightmap(TPE_Vec3 point, 
                          TPE_Vec3 center, 
                          TPE_Unit gridSize,
                          TPE_Unit (*heightFunction)(int32_t x, int32_t y), 
                          TPE_Unit maxDist);
...
TPE_Unit TPE_bodyGetNetSpeed(const TPE_Body *body)
{
  ...
  for (i = 0; i < body->jointCount; ++i)
  {
    velocity += TPE_LENGTH(TPE_vec3(joint->velocity[0],
                                    joint->velocity[1],
                                    joint->velocity[2]));
                               ...
                               TPE_Unit speedMax)
{
  ...
  for(i = 0; i < 16; ++i)
  {
    ...
    fraction = (((speedMax + speedMin) / 2) * TPE_F) /
      TPE_nonZero(speed);
...
void TPE_bodyCancelOutVelocities(TPE_Body *body, uint8_t strong)
{
  ...
  for(i = 0; i < body->connectionCount; ++i)
  {
    ...
    if (cancel)
    {
      ...
      TPE_Vec3 v1 = TPE_vec3(j1->velocity[0],j1->velocity[1],j1->velocity[2]),
                                   ...
                                   TPE_ClosestPointFunction env)
{
  ...
  if (d < 0) // collision?
  {
    ...
    ratio = (mass2 * TPE_F) / TPE_nonZero(mass1 + mass2);
    ...
    frictionVec = TPE_vec3(j1->velocity[0],
                           j1->velocity[1],
                           j1->velocity[2]);
  ...
  TPE_Unit elasticity, TPE_Unit friction, TPE_ClosestPointFunction env)
{
  ...
  if (len <= TPE_JOINT_SIZE(*joint))
  {
    ...
    if (success)
    {
      ...
      vel = TPE_vec3(joint->velocity[0],joint->velocity[1],
        joint->velocity[2]);
...
#define Z_LIMIT 250
  ...
  for(i = 0; i < world->bodyCount; ++i)
  {
    ...
    for(j = 0; j < world->bodies[i].connectionCount; ++j)
    {
      ...
      TPE_Vec3 p1 = world->bodies[i].joints[world->bodies[i].connections[j].joint1].position,
...
TPE_Vec3 TPE_envAABox(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 maxCornerVec)
{
  ...
  region = (shifted.x > maxCornerVec.x) |
...
TPE_Vec3 TPE_envHalfPlane(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 normal)
{
  ...
  TPE_Unit tmp = point2.x * normal.x + point2.y * 
                 normal.y + point2.z * normal.z;
  ...
  return TPE_vec3Minus(point, TPE_vec3Times(normal,tmp));
}
                          ...
                          TPE_Unit maxDist)
{
  TPE_Vec3 closestP;
  TPE_Unit closestD;
  int16_t startSquareX, startSquareY;
