...
#include<3rdparty/nanojpeg/nanojpeg.h>
...
GLuint load_texture_png(const char *filename, GLenum format, GLenum pixelType) 
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, format, pixelType, image);
...
GLuint load_texture_jpeg(const char *filename, GLenum format, GLenum pixelType) 
{
    ...
    if (!buffer) 
    {
        printf("Erro: Falha ao alocar memória para o buffer\n");
        fclose(file);
        return 0;
    }
    ...
    njInit();
    if(njDecode(buffer, size)) 
    {
        printf("Erro: Falha ao decodificar o arquivo JPEG %s\n", filename);
        free(buffer);
        return 0;
    }
    ...
    image = njGetImage(); // Dados da imagem em RGB
    width = njGetWidth();
    height = njGetHeight();
    components = njGetImageSize() / (width * height); // Componentes por pixel (ex: 3 para RGB)

    if(components != 3) 
    {
        printf("Erro: A imagem não é RGB\n");
        njDone();
        return 0;
    }

    // Criar e carregar a textura no OpenGL
    glGenTextures(1, &texture);
    glBindTexture(GL_TEXTURE_2D, texture);

    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, format, pixelType, image);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    // Limpar o decodificador
    njDone();
