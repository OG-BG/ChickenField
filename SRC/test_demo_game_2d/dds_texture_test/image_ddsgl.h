 ...
 * \return Nonzero on success.
 */
#define GLAD_EXT_texture_compression_s3tc 1

static int liimg_ddsgl_load_levels(LIImgDDS*         dds,
								   ...
								   GLuint            texture)
{
	...
	for (level = 0 ; !level || level < (int) dds->header.mipmaps ; level++)
	{
		...
		else if(GLAD_EXT_texture_compression_s3tc)
		{
			/* Compressed format. */
			if(!liimg_dds_read_level(dds, file, level, &lvl))
			...
			glCompressedTexImage2DARB(target, level, 
									  format->format,
									  lvl.width, 
									  lvl.height, 
									  0, 
 ...
 */
static GLuint liimg_ddsgl_load_texture (FILE*     file,
	LIImgDDS* info)
{
	int i;
	GLuint texture;
