...
#include<stdbool.h>
#include<stdint.h>
#include"../model_test/ANOTHER/system/system_endian.h"
#include"../model_test/ANOTHER/system/system_memory.h"
 ...
 */
static void liimg_dds_init_rgba(LIImgDDS* self,
								int       width,
								int       height,
								int       mipmaps)
{
 ...
 * \param mipmaps Mipmap count.
 */
static void liimg_dds_init_s3tc (
	LIImgDDS* self,
	int       width,
	int       height,
	int       size,
 ...
 * \param self DDS.
 */
static void liimg_dds_byteswap(LIImgDDS* self)
{
	if(lisys_endian_big())
	{
...
int liimg_dds_decompress(const void* input,
						 void*       output,
						 int         width,
						 int         height,
						 int         format);
 ...
 */
static int liimg_dds_read_header(LIImgDDS* self,
								 FILE*     file)
{
	int i;
	int alpha = 0;
 ...
 * \return Nonzero on success.
 */
static int liimg_dds_read_level(LIImgDDS*      self,
								FILE*          file,
								int            level,
 ...
 * \return Nonzero on success.
 */
static int liimg_dds_write_header(LIImgDDS* self,
								  FILE*     file)
{
	liimg_dds_byteswap (self);
 ...
 * \return Nonzero on success.
 */
static int liimg_dds_write_level(LIImgDDS* self,
								 FILE*     file,
								 int       level,
								 void*     data,
								 int       size)
