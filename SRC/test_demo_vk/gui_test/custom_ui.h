...
UI_Rectangle GetLayoutBounds(Layout *layout);
...
int GetLayoutHeigth(Layout *layout);
UI_Rectangle GetWidgetBounds(Widget *widget);
...
Widget *CreateComboBox(UI_Rectangle rect, char **options, size_t numOptions);
Widget *CreateGroupBox(UI_Rectangle rect, char *title);
Widget *CreateSeparator(int x, int y, int width);
Widget *CreateTextBox(UI_Rectangle rect);
Widget *CreateMenuBar();
Widget *CreateMenuItem(Widget *parentMenuItemWidget, char *text);
Widget *CreateListView(UI_Rectangle rect);
...
static void DrawTextCentered(char *text, int fontSize, UI_Rectangle rect);
...
{
    Vector2 result;
    result.x = layout->rect.x;
    result.y = layout->rect.y;
    return result;
...
char* GetTextBoxText(Widget *textBoxWidget) 
{
    ...
    textbox = (TextBox*)textBoxWidget->component;
    return textbox->text;
}
...
char* GetComboBoxOption(Widget *comboBoxWidget) 
{
    ...
    combobox = (ComboBox*)comboBoxWidget->component;
...
void ClearListView(Widget *listViewWidget) 
{
    ...
    listview = (ListView*)listViewWidget->component;
    listview->count = 0;
}
...
void SetLabelColor(Widget *labelWidget, Color color) 
{
    ...
    label = (Label*)labelWidget->component;
    label->color = color;
}
