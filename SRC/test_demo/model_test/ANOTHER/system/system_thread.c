...
static void* private_thread(void* data);

/*****************************************************************************/

LISysThread* lisys_thread_new(LISysThreadFunc func,
							  void*           data)
{
	LISysThread* self;
...
void lisys_thread_free(LISysThread* self)
{
	lisys_thread_join (self);
	lisys_free (self);
}

void lisys_thread_join(LISysThread* self)
{
	pthread_join (self->thread, NULL);
}

int lisys_thread_get_done(LISysThread* self)
{
	return self->done;
}
