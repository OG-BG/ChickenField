...
#include<string.h>
#include"test_demo/model_test/ANOTHER/system/system_compiler.h"
#include"test_demo/model_test/ANOTHER/system/system_mmap.h"
#include"archive_packet.h"

LIArcPacket* liarc_packet_new_readable(const char* buffer,
									   int length)
{
	LIArcPacket* self;
...
LIArcPacket* liarc_packet_new_readable_copy(const LIArcPacket* packet)
{
	int length;
	LIArcPacket* self;
...
LIArcPacket* liarc_packet_new_writable(int type)
{
	LIArcPacket* self;
...
void liarc_packet_free(LIArcPacket* self)
{
	if(self->writer != NULL)
