...
int limdl_partition_init_copy (LIMdlPartition*       self,
...
void limdl_partition_clear (LIMdlPartition*  self)
{
	lisys_free (self->vertices.array);
	lisys_free (self->name);
}

int limdl_partition_read(LIMdlPartition* self,
						 LIArcReader*    reader)
{
	int i;
	uint32_t tmp;
...
int limdl_partition_write(const LIMdlPartition* self,
						  LIArcWriter*          writer)
{
	int i;
